...
export default function MapLibreWrapper({ initialCenter = [73.8567, 18.5204], routeData, driverLocation }) {
  ...
  useEffect(() => {
    ...
    // Use Carto Dark Matter strictly for the premium Movbot deep dark UI
    map.current = new maplibregl.Map({
      container: mapContainer.current,
      style: 'https://basemaps.cartocdn.com/gl/dark-matter-gl-style/style.json',
      center: initialCenter,
      zoom: 14,
      pitch: 45, // 3D angled view like the screenshot
      bearing: 0,
  ...
  useEffect(() => {
    ...
    if (!driverMarkerRef.current) {
      // Create custom SVG element for the Truck/Driver
      const el = document.createElement('div');
      el.className = 'w-10 h-10 bg-white rounded-xl shadow-[0_0_20px_rgba(255,255,255,0.4)] flex items-center justify-center border border-surfaceDarker rotate-[-45deg] transition-all duration-1000';
      ...
      driverMarkerRef.current = new maplibregl.Marker({ element: el, pitchAlignment: 'map' })
  ...
  return (
    <div className="absolute inset-0 z-0">
      ...
      {mapLoaded && routeData && (
        <MintRouteLayer map={map.current} routeCoordinates={routeData} />
      )}
    </div>
  );
}