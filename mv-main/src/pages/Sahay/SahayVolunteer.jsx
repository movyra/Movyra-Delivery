            ...
            setCases(cases.map(c => {
                if (c.id === caseId) {
                    return { 
                        ...c, 
                        status: 'Assigned',
                        volunteersAssisting: [...c.volunteersAssisting, currentUser.uid] 
                    };
                }
                return c;
            }));
            ...
            <main className="flex-1 max-w-[800px] w-full mx-auto px-6 md:px-12 py-12 animate-fade flex flex-col">
                ...
                <div className="flex gap-4 mb-8 border-b border-[#E5E7EB]">
                    <button 
                        ...
                        className={`pb-4 px-2 font-black text-[1.1rem] outline-none transition-colors relative ${activeTab === 'Available' ? 'text-[#111111]' : 'text-[#555555] hover:text-[#111111]'}`}
                    >
                        {currentT.tab_avail}
                        {activeTab === 'Available' && <motion.div layoutId="tabIndicator" className="absolute bottom-0 left-0 right-0 h-1 bg-[#111111]" />}
                    </button>
                    <button 
                        onClick={() => setActiveTab('Mine')}
                        className={`pb-4 px-2 font-black text-[1.1rem] outline-none transition-colors relative ${activeTab === 'Mine' ? 'text-[#111111]' : 'text-[#555555] hover:text-[#111111]'}`}
                    >
                        {currentT.tab_mine}
                        {activeTab === 'Mine' && <motion.div layoutId="tabIndicator" className="absolute bottom-0 left-0 right-0 h-1 bg-[#111111]" />}
                    </button>
                </div>
                ...
                {isLoading ? (
                    ...
                    <div className="rounded-3xl p-12 text-center border border-dashed border-[#D1D5DB] bg-[#F7F7F7] flex flex-col items-center justify-center">
                        ...
                        <h2 className="text-[1.5rem] font-black tracking-tight mb-2 text-[#111111]">
                            {activeTab === 'Available' ? currentT.empty_avail : currentT.empty_mine}
                        </h2>
                    </div>
                ) : (
