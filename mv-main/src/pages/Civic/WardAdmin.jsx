...
export default function WardAdmin() {
    ...
    const deleteIncident = async (incidentId) => {
        ...
        setProcessingId(incidentId);
        try {
            await deleteDoc(doc(db, 'civic_complaints', incidentId));
            setIncidents(incidents.filter(i => i.id !== incidentId));
        } catch (error) {
            console.error("Deletion failed:", error);
            alert("Authorization failed. Ensure you have super admin privileges.");
        } finally {
            setProcessingId(null);
        }
    };
                                            ...
                                            <tr key={incident.id} className={`border-b transition-colors ${theme === 'light' ? 'border-[#e0e0e0] hover:bg-[#f5f5f5]' : 'border-[#333333] hover:bg-[#1a1a1a]'}`}>
                                                ...
                                                <td className="p-6 align-top text-right">
                                                    <div className="flex flex-col gap-2 items-end">
                                                        ...
                                                        {activeUser?.email === 'testcodecfg@gmail.com' && (
                                                            <button 
                                                                onClick={() => deleteIncident(incident.id)}
                                                                disabled={processingId === incident.id}
                                                                className={`mt-2 flex items-center gap-1 px-4 py-2 font-bold text-[0.8rem] rounded-lg transition-colors disabled:opacity-50 outline-none border ${
                                                                    theme === 'light' ? 'bg-[#ffebee] border-[#ffcccc] text-[#cc0000] hover:bg-[#ffcccc]' : 'bg-[#330000] border-[#660000] text-[#ff4444] hover:bg-[#440000]'
                                                                }`}
                                                            >
                                                                <Trash2 size={14} /> {currentT.act_del}
                                                            </button>
                                                        )}
                                                    </div>
                                                </td>
